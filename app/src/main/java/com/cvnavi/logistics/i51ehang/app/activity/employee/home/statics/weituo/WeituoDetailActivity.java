...
public class WeituoDetailActivity extends BaseActivity {
    ...
    public static void start(Context context, String ticketNo) {
        Intent starter = new Intent(context, WeituoDetailActivity.class);
    ...
    private void getDestinatInfo() {
        ...
        VolleyManager.newInstance().PostJsonRequest(EmployeeService.GetDelegateDetailByOid_Url, EmployeeService.GetDelegateDetailByOid_Url, GsonUtil.newInstance().toJson(dataRequestBase), new Response.Listener<JSONObject>() {
            @Override
            public void onResponse(JSONObject response) {
                ...
                if (result != null && result.isSuccess() && result.getDataValue() != null) {
                    ...
                    if (listBeen != null && listBeen.size() > 0) {
                       ...
                       final SendCarDetaiItemModel.DataValueBean.StowageListBean info = listBeen.get(0);
                        ...
                        if (!TextUtils.isEmpty(info.getThreeCompany())) {
                            company.setText(info.getThreeCompany());
                        } else {
                            company.setText("--");
                        }
                        ...
                        if (!TextUtils.isEmpty(info.getDriver_Tel())) {
                            ...
                            tel.setOnClickListener(new View.OnClickListener() {
                                ...
                                public void onClick(View v) {
                                    ContextUtil.callAlertDialog(info.getDriver_Tel(), WeituoDetailActivity.this);
                                }
                            });
                        } else {
                            tel.setText("--");
                        ...
                        if (!TextUtils.isEmpty(info.getAllTicket_Count())) {
                            piao.setText("票数：" + info.getAllTicket_Count()+"票");
                        } else {
                            piao.setText("票数：" + "--");
                        }

                        if (!TextUtils.isEmpty(info.getAllBulk_Weight())) {
                        ...
                        if (!TextUtils.isEmpty(info.getOperate_DateTime())) {
                            time.setText("委托时间："+info.getOperate_DateTime());
                        } else {
                            time.setText("委托时间："+"--");
                        }
                    }
                     ...
                     */
                    if (adapter == null) {
                        adapter = new WeiTuoDetailAdapter(WeituoDetailActivity.this, mainTicketBeen,WeiTuoDetailAdapter.FROM_WEI_TUO);
                    ...
                    DialogUtils.showMessageDialogOfDefaultSingleBtnNoCancel(WeituoDetailActivity.this, "获取数据失败", new CustomDialogListener() {
                        @Override
            ...
            public void onErrorResponse(VolleyError error) {
                dissLoading();
                DialogUtils.showMessageDialogOfDefaultSingleBtnNoCancel(WeituoDetailActivity.this, "网络异常", new CustomDialogListener() {
                    @Override
                    public void onDialogClosed(int closeType) {
                        finish();
                    }
